...
gears lines =
    let symbols' = symbols (== '*') (toIndexed lines)
        numbers' = numbers (toIndexed lines)
        allAdjacent p = filter (\(_, region) -> adjacent region p) numbers'
    in map (\[(a, _), (b, _)] -> (a, b)) $ filter (\xs -> length xs == 2) $ map allAdjacent symbols'
...
main :: IO ()
main = interact' (sum . map gearRatio . gears . lines)
